...
function build_api(){
    ...
    docker buildx build --platform linux/amd64 -f ./Dockerfile --build-arg envarg=$envarg -t ${DOCKER_REPO:-'local'}/openreplay:chalice-${git_sha1} .
    cd ../api
    rm -rf ../_api
    [[ $PUSH_IMAGE -eq 1 ]] && {
        docker push ${DOCKER_REPO:-'local'}/openreplay:chalice-${git_sha1}
        docker tag ${DOCKER_REPO:-'local'}/openreplay:chalice-${git_sha1} ${DOCKER_REPO:-'local'}/openreplay:chalice-${tag}latest
        docker push ${DOCKER_REPO:-'local'}/openreplay:chalice-${tag}latest
    }
